...
This thesis mainly considers \emph{programmed} agents, as the goal is using agents to encapsulate business logic that can automate processes and activities in the context of \ac{DT}-based systems. 
%
...
%
\ac{BDI} agents are the central focus of the chapter, as they are amongst the most widely adopted agent model for programmed agents, capable of encoding domain and procedural knowledge through the cognitive abstractions they provide, thus making \ac{BDI} agents excellent candidates for reliable and explainable intelligent applications. 
...
%
Namely, an agent is \emph{situated} in an environment which can be either physical (e.g., for a robotic system) or virtual (e.g., for a software agent).
%
...
\begin{figure}[tb]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/agent-environment-original.pdf}
...
%
How this is achieved depends on the agent architecture and model adopted, but it is possible to generally distinguish two main categories: \emph{programmed} or \emph{learning} agents, depending on whether their behavior is explicitly defined by a developer or learned from data and experience. 
%
...
Considering this very broad landscape, for the scope of this thesis the term \emph{agent} is used to implicitly refer to \emph{programmed agents} and specifically on \ac{BDI} agents as an example of systems with a \emph{strong} notion of agency.
%
...
%
When discussing the engineering of \acp{MAS} in this thesis, a \ac{MAOP}-inspired approach is adopted, with a focus on the agent and environment dimensions (and the A\&A metamodel in particular), as these are most relevant for the integration of \acp{MAS} with \ac{DT}-based systems.
%
While the organization dimension is also of interest, it is not specifically addressed here, as the focus is on relatively small-scale \acp{MAS} where the organization remains implicit.
...
\begin{figure}[tb]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/bdi-agent-schema.png}
...
% \todo{explain why consider agentspeak and jason here}
...
\begin{figure}[tb]
    \centering
    \includegraphics[width=\textwidth]{figures/agent-dt-overview.pdf}
    \caption{Schematic representation of the integration between \acp{MAS} and \acp{DT} through the environment dimension. \acp{DT} can be seen as artifacts which the agents interact with, from \cite{Mariani_Picone_Ricci_2022}.}
...
Both perspectives are relevant for this thesis, as the goal is to explore how to engineer intelligent applications that rely on \acp{DTE} to interact with the physical world. 
%
