...
\begin{figure}[tb]
    ...
    \caption{A side-by-side mapping between 5D DT modelling (left) and the abstract \ac{DT} architecture presented in this section (right). The architecture is presented with named boxes that highlight the main components of a \ac{DT} and its interaction with the \ac{PA} and external applications.}
    \label{fig:tao_mapping_dt_modelling}
\end{figure}
%%%

In the 5D model (Figure \ref{fig:tao_mapping_dt_modelling}, left), 
the first dimension, Physical Entitiy (PE), represents the actual physical object or system being modeled as a \ac{DT}.
...
\Cref{tab:evaluation} discusses how the proposed architecture supports the core properties of \acp{DT}~\cite{dt-IoT-context-Minerva-2020} compared to both existing research from the scientific literature and features offered by major production-ready platforms.
%
Existing \ac{DT} cloud and vendor-specific platforms -- such as the ones described in \Cref{sec:back:dt:technology} -- either mostly serve as data repositories, storing data from physical assets, with pre-processing handled externally by a combination of services, or implement monolithic solutions with limited flexibility and adaptability to different application contexts.
%
...
\begin{equation}\label{eq:dt_state}
...
Finally, considering these additional specifications, the resulting \ac{DT} model enhances (Eq.~\ref{eq:dt_basic_model}) to include also the shadowing $Shad$ and the history of states $H$ in the \ac{DT} definition:
...
\begin{figure}[tb]
    ...
    \caption{Example of a \ac{DT} in an application scenario interacting with a physical asset and digital applications through its \ac{PI} and \ac{DI}. The example showcases the presence of multiple communication channels on both sides to highlight the heterogeneity of physical and digital interactions.}
...
\begin{figure}[tb]
    ...
    \caption{\acl{PI} design with modular physical adapters each producing the corresponding \acl{PAD} that is processed by the \ac{DT} Model.}
...
\begin{figure}[tb]
    \centering
    \includegraphics[width=0.9\columnwidth]{figures/dt-interoperability/dt_interoperability_digital.pdf}
    \caption{\acl{DI} design with modular adapters. Each adapter publishes a description of the state, events and actions it supports so that consumers can dynamically understand and interact with the \ac{DT}.}
...
%
A path to address these limitations could be extending \acp{TD} or supporting custom descriptions for \acp{DT} in \acp{DTE} as explored in \Cref{chap:dte:hwodt}.
...
\begin{figure}[tb]
    \centering
    \includegraphics[width=0.75\columnwidth]{figures/wldt_lifecycle_simple.pdf}
    \caption{DT Life Cycle with its phases and transitions. Circles represent phases, rounded squares represent terminal phases, and arrows represented transitions. Phases are grouped in three major operating states of the \ac{DT} which are \texttt{Starting}, \texttt{Working} and \texttt{Stopping}.}
...
Upon creation (from the \texttt{Started} phase \circled{1}),
the \ac{DT} enters the \texttt{Unbound} phase \circled{2}, where all internal modules are initialized and prepared for the binding process with the \ac{PA}.
%
...
the \ac{DT} enters the \texttt{Bound} phase \circled{3}.
%
During this phase, the \ac{DT} is connected to the \ac{PA} and is ready to begin the shadowing process.

The next phase is the \texttt{Synchronized} phase \circled{4}, 
...
If any issues arise, such as network failures, and the \ac{DT} synchronization performance falls under the expected requirements, the \ac{DT} enters the \texttt{Out of Sync} phase \circled{5}.
...
Finally, when the \ac{DT} is no longer required or has completed its function, it transitions to the \texttt{Done} phase \circled{6}.
In this phase, the \ac{DT} remains accessible to external consumers and retains its memory, but it is no longer bound to the \ac{PA} or in sync with it. At the end of the lifecycle, the \ac{DT} can be dismissed and moved to the \texttt{Stop} phase \circled{7}.
The \ac{DT} may also return to the \texttt{Unbound} phase if errors are detected during the binding process or if it is rebooted or restarted.
...
\begin{figure}[tb]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/dt-lifecycle-zoom.pdf}
    \caption{Zoomed view on the \ac{DT} lifecycle with a structured example of the \texttt{Synchronized} phase from \Cref{fig:dt_life_cycle}. The images shows that within the \texttt{Synchronized} phase, the \ac{DT} reflects the \ac{PA}'s operational phases, such as \texttt{Ready}, \texttt{Working}, \texttt{Busy}.}
...
Here the focus is on representing an asset in terms of its present (and past) state, hence the \ac{DT} representation $R_{DT}$ is considered as a function of time.
%
Recalling Eq.~\ref{eq:dt_state} the state includes all relevant properties, actions, events and relationships of the \ac{DT} at a given time instant $t_i$.
%
Other relevant aspects such as \ac{DT} models and interfaces are not considered here for the sake of simplicity, but they can be easily included in the representation if needed, and they may evolve over time as well, depending on the lifecycle phase.
%
For instance, actions in the \ac{DT} state might be enabled or disabled depending on the lifecycle phase (e.g., in the \texttt{Done} phase, no actions can be executed), and with those, also the \ac{DT} interface may change to expose only the available functionalities.

The \ac{DT} representation of the asset $R_{DT}$ can hence be composed by the \ac{DT}'s lifecycle phase $LP_{DT}(t_i)$, its current state $S_{DT}(t_i)$, and its history $H(t_a, t_b)$ over a specific time interval $(t_a, t_b)$, as shown in Eq.~\ref{eq:dt-definition}.
...
%
Note that, since the \ac{DT} history is computed only when the \ac{DT} is synchronized, there might be cases in which the state at time $t_i$ shows only a partial history and hence in Eq.~\ref{eq:dt-definition} $t_b$ may be different from $t_i$.
...
\begin{figure}[tb]
    ...
    \caption{Schematic Event-Driven architecture of a \ac{DT} to support Augmentation function management and execution. A central event bus supports asynchronous interactions between the shadowing and augmentation functions. Functions can access the \ac{DT} models and history as needed.}
...
%=======================================================
\section{Extending the \acl{WLDT} Framework}
\label{sec:dte:engineering-dt:wldt-framework}
%=======================================================

The concepts introduced in the previous sections have been realized through a substantial extension and architectural refactoring of the \acf{WLDT} framework, originally introduced in \cite{PICONE2021100661}.
%
Rather than presenting WLDT as a new contribution, this section documents how the framework has been redesigned and evolved the \ac{DT} engineering principles and lifecycle requirements introduced above.

WLDT is an open-source Java framework available on GitHub\footnote{\url{https://wldt.github.io}}, conceived to support the development of \acp{DT} with a focus on \ac{IoT}-enabled applications.
%
The work presented here extends its features by introducing explicit support for \ac{DT} lifecycle management, separation of shadowing and augmentation concerns, and modular interfaces aligned with distributed and microservice-oriented deployments.
%
The extensions are designed to use \ac{WLDT} as a concrete implementation of the abstract DT architecture and lifecycle model proposed in this thesis, serving as a building block for the development of \acp{DTE}.

\begin{figure}[tb]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/wldt_architecture.pdf}
    \caption{Extended WLDT software architecture aligned with the DT engineering model proposed in this thesis.}
    \label{fig:wldt_architecture}
\end{figure}

The redesign of WLDT has been guided by three main requirements derived from the thesis contributions:
\begin{itemize}
    \item \textbf{Simplicity}, to support the rapid engineering of \acp{DT} through reusable modules and configuration-based customization;
    \item \textbf{Extensibility}, to enable the concurrent execution of multiple behaviors and the incremental evolution of \ac{DT} capabilities;
    \item \textbf{Portability and microservice readiness}, to allow \acp{DT} to be deployed as independent computational entities across heterogeneous infrastructures.
\end{itemize}

\paragraph{Architectural refactoring.}
The extended WLDT architecture, shown in \Cref{fig:wldt_architecture}, is organized around four conceptual layers: a core execution library, a modular DT model, a core shared adapter logic, and a set of community-developed physical and digital adapters.
%
This structure directly reflects the abstract DT architecture introduced in \Cref{sec:dte:engineering-dt:abstract-architecture}, providing a concrete realization of its main concepts.
%
The core library provides execution control, lifecycle coordination, and internal event-driven communication mechanisms.
%
Supporting services such as logging, metrics, and common utilities improve observability and robustness of \ac{DT} instances. 

\paragraph{Digital Twin core and lifecycle support.}
A key contribution of this work is the refactoring of the DT core into an explicit \emph{Model} component that governs the evolution of the DT over time.
%
This model controls both the \emph{Shadowing Function} and one or more \emph{Augmentation Functions}, making their roles explicit and separable.
%
The Shadowing Function is responsible for maintaining alignment between the physical asset and its digital counterpart by processing data received from physical adapters and action requests issued through digital adapters.
%
It manages the DT \emph{state} in terms of properties, events, actions, and relationships, and drives the transitions between lifecycle phases as introduced in \Cref{sec:dte:engineering-dt:dt-lifecycle}.
%
The state management component is designed to support transactional updates and maintain a history of state changes, enabling both real-time synchronization and retrospective analysis.
%
The shadowing function determines when a DT can be considered \texttt{Bound} and when it reaches a \texttt{Synchronized} state suitable for higher-level reasoning and interaction.
%
Augmentation Functions encapsulate additional behaviors that go beyond pure mirroring, such as data aggregation, anomaly detection, or domain-specific processing.
%
This separation enables the incremental enrichment of DT capabilities while preserving a clear boundary between physical synchronization and added intelligence.

\paragraph{Adapters as interaction boundaries.}
WLDT adopts a modular adapter-based approach to interface \acp{DT} with both the physical and digital worlds.
%
Physical Adapters handle the ingestion of data and commands from heterogeneous physical assets, while Digital Adapters expose DT state and capabilities to external consumers through application-specific protocols.
%
Adapters are managed as first-class components with dedicated lifecycles and unique identifiers, allowing multiple adapters to coexist and be coordinated within the same DT instance.
%
Digital Adapters can access both the current DT state and its history, enabling the exposure of richer digital services without embedding protocol-specific logic into the DT core.

\paragraph{Execution and scalability considerations.}
The extended WLDT framework includes a multi-threaded execution engine capable of managing multiple DT instances within the same runtime.
%
While the current implementation targets single-process execution, the abstraction is intentionally aligned with distributed and microservice-based deployments, paving the way for future extensions where DTs may be deployed across multiple processes or nodes within a \ac{DTE}.

\medskip
Overall, the extensions presented in this section transform WLDT from a framework primarily aimed at individual \ac{IoT}-focused \acp{DT} into a more comprehensive framework that explicitly supports the engineering principles and lifecycle management strategies that separate responsibilities of a \ac{DT} across modular and reusable components.
...
\Cref{fig:dt-assets-relationships-overview} illustrates the relationships among the various \acp{DT} in the Multiprocess Station.
%
Relationships between machines and DTs (e.g., \emph{previous}, \emph{following-machines}, \emph{is-part-of}) are statically defined in this setting and used by department-level \acp{DT} to track and compute throughput and total pieces processed.
%
\begin{figure}[tb]
    \centering
    \includegraphics[width=\textwidth]{figures/dt-interoperability/mf_dt_structure.pdf}
    \caption{The \acl{DT} ecosystem architecture of the Indexed Department, showing the different \acf{PhA} and \acf{DiA} of each \ac{DT} alongside a schematic representation of the conveyors.}
...
\begin{figure}[tb]
  ...
  \begin{subfigure}[t]{0.42\linewidth}
      ...
      \caption{Indexed-Line Digital Twin}
  ...
  \end{subfigure}
  \caption{Cyber-physical complexity computed on different levels of the hierarchical \ac{DT} system implemented for the microfactory, showing reduced complexity compared to an approach without modular \acp{DT} interfaces.}
...
\begin{figure}[tb]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/experimental_evaluation_scheme_new_industry.pdf}
    \caption{Experimental setup for evaluating the design of augmentation functions for a pose-estimation task in a \ac{DT} of a human operator.}
...
\paragraph{\ref{rq:1} How to engineer \acp{DT} which integrate heterogeneous data sources and provide an up-to-date uniform representation of the mirrored assets?}
%
