...
%
This thesis focuses on the software engineering perspective of \acp{DT}, 
...
\begin{figure}[tb]
    ...
    \caption{The original \ac{DT} conceptual model by Grieves, from \cite{Grieves_2022}.}
...
the following definition is adopted, starting from the one in \cite{dt-IoT-context-Minerva-2020}:
...
%
The term \emph{\ac{PA}} is deliberately used to refer to the physical counterpart of a \ac{DT}
...
%
Compared to the original Grieves model which characterized the \ac{DT} as having three parts (physical, digital and connection), the remainder of this thesis will instead consider:
\begin{itemize}
...
\begin{itemize}
\item the \emph{physical entity} (PE), as the entity in the physical world that is being represented by the \ac{DT} (i.e., what is identified as \ac{PA} in this thesis);
...
\begin{figure}[tb]
    \centering
    \includegraphics[width=0.55\textwidth]{figures/5d-model.pdf}
    \caption{Conceptual representation of the 5D model of a Digital Twin, showing the five dimensions and the connections between them.}
...
%
Many of these tools, emerging from industrial domains, are often domain-specific and provide strong integration with the physical assets and systems produced by the same companies.
...
%
Additionally, being proprietary software, they often lack openness and interoperability becoming closed silos from a broader system engineering perspective as the competitive nature of the market tends to prefer both hardware and software lock-in instead of promoting integration.

More recently, cloud providers have started to offer so-called \emph{\ac{DT} platforms}.
%
Prominent examples are Microsoft's \azureTwin{}\footnote{\url{https://azure.microsoft.com/en-us/services/digital-twin/}} and Amazon's \awsTwin{}\footnote{\url{https://aws.amazon.com/iot-twinmaker/}}.
%
These general-purpose cloud platforms provide an integration layer on top of existing cloud data storage and \ac{IoT} middlewares and hence mainly function as data repositories, storing data from physical assets, with pre-processing handled externally.
%
They complement the existing cloud-based service ecosystem with specific features to support the modeling of \acp{DT}.
%
...
%
Data acquisition pipelines, supported protocols and data formats are often defined at the platform level, not on a per-\ac{DT} basis, leading to limitations in defining clear boundaries for the \ac{DT} digitalization process and client interactions.

Interestingly, though, the broader scope of cloud platforms put forth the idea of having multiple \acp{DT} connected together to represent a complex application domain. 
%
...
Finally, open-source \ac{DT} platforms are emerging~\cite{Gil_Mikkelsen_Gomes_Larsen_2024}.
...
%
\Cref{sec:dte:hwodt:related} will present an analysis of the positioning of this thesis' contribution with respect to the state of the art in \ac{DT} platforms. 
...
\begin{figure}[tb]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/AI_DT_pentagon.pdf}
...
\begin{figure}[tb]
    ...
    \caption{An overview of \ac{DT} for healthcare, showing technological enablers, target goals and the main axis that are relevant for healthcare \acp{DT}, namely time and scale. Image from \cite{Katsoulakis_Wang_Wu_Shahriyari_Fletcher_Liu_Achenie_Liu_Jackson_Xiao_et_al._2024}.}
...
This thesis focuses on the use of \ac{DT} to model healthcare processes and workflows, as a more pragmatic approach to leverage the potential of \ac{DT} in healthcare, while addressing the needs of Healthcare 4.0 healthcare systems.
...
\begin{figure}[tb]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/wodt.pdf}
    \caption{Conceptual model of the \acl{WoDT}. \aclp{PA} (squares) have corresponding \aclp{DT} (circles) connected through relationships that mirror the ones in the physical world. 
    Applications and users (top) can navigate, query and observe the \acl{WoDT} to access data and services from the interconnected \aclp{DT}.}
