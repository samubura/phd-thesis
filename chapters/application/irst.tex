...
For this case study, the proposed \ac{DTE} model has not been fully implemented;
however, parts of the system have been prototyped to demonstrate the feasibility of the approach and explore its potential.

To represent the data collected by the system, a \ac{KG} has been developed to model the various entities and their relationships.
...
The analysis and prototype design presented in this chapter demonstrate the potential of \acp{DTE} to address scenarios that involve complex processes that span across multiple heterogeneous physical assets.
%
The scenario includes \acp{DT} that track people (patients), processes (treatments), and infrastructures (production units and warehouses), each with its own dynamics and interactions.
%
By modeling these entities as \acp{DT}, it is possible to create a comprehensive digital representation of the entire pharmaceutical supply chain for oncologic treatments.

The case study is meant to showcase how the healthcare domain may require tackling complex business processes that go beyond the clinical aspects of patient care, involving logistics, resource management and process optimization.
%
In this context, \acp{DTE} can provide a uniform approach to model and manage the ecosystem of heterogeneous assets involved. The \ac{KG}-based model allows to represent the various entities and their relationships with clear, standardized semantics, enabling interoperability and data integration.

Although the full implementation of the proposed \ac{DTE} model has not been realized, the design has proven feasible and useful for the administration to conceptually understand and analyze the role that digital technologies can play in optimizing the pharmaceutical supply chain for oncologic treatments.
