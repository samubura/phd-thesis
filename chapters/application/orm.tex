...
This section describes a \ac{DTE} designed to collect data and monitor ongoing surgeries in support of ORM.
Assuming the adoption of \acp{DT} as a pervasive approach, an ecosystem of connected \acp{DT} can be envisioned to represent real-world facilities.
Each hospital can be represented by a \ac{DT}, which is connected to the \acp{DT} of each operating room.
These, in turn, can be linked to the \acp{DT} of the medical equipment available in each room.
Similarly, individuals, such as patients and medical staff, can be mirrored to track their interactions with facilities and with each other.

\begin{figure}[htb]
    \centering
    \includegraphics[width=0.8\columnwidth]{figures/orm/DTORM.pdf}
...
\begin{figure}[hp]
    ...
    \caption{The final FHIR resources mapping data related to operating rooms and procedures. Each box represent a FHIR resource, while arrows represent fields holding references between resources.}
...
%\Cref{fig:orm:anylogic} and 
\Cref{fig:orm:anylogic2}, an agent-based simulation developed in AnyLogic\footnote{\url{https://www.anylogic.com/}} has been used to model the patient flow through the operating rooms and generate synthetic data that can feed a prototype implementation of the \ac{DTE}.
...
% \begin{figure}[ht]
%     \centering
%     \includegraphics[width=0.9\textwidth]{figures/orm/anylogic.png}
