...
A \ac{DTE} can be considered a \emph{Homogeneous} \acp{DTE} when the approach relies on a general-purpose platform that supports the definition of multiple \acp{DT} (\Cref{fig:ecosystem-homogeneous}).
%
The \acp{DT} are hence homogeneous in their modeling and implementing technology as they are created on the same platform.
%
...
%
Furthermore, the modeling and technological homogeneity make implementing ecosystem services relatively easy, similarly to the \emph{monolithic} approach.
%
Differently, though, it is possible to highlight the individuality of the involved entities and their corresponding \acp{DT}, which may independently process the relative data flows, achieving better separation of concerns.
