...
The last decade has been marked by a significant transformation driven by the integration of advanced digital technologies such as the \ac{IoT}, \ac{AI} and \ac{ML} to improve the efficiency and automation of processes across manufacturing and service industries.
...
This exploration follows the rising trend of digitalizing the core assets of an application domain with \emph{\acp{DT}}: representations of \acp{PA}, connected and synchronized through data streams from sensors and other digital sources~\cite{Grieves_2023}.
%
The \ac{DT} concept has emerged as a paradigm shift in how to model and interact with the physical world when designing \acp{CPS}, focusing on capturing the essential features of each relevant entity, creating personalized models and combining real-time data with modeling techniques, \ac{AI} and simulation~\cite{Semeraro_Lezoche_Panetto_Dassisti_2021}.
%
...
%
This thesis, proposes a framework for the development of \emph{\acp{DTE}} that can incorporate heterogeneous \acp{DT} and expose a holistic, synchronized view of the mirrored physical environment to applications under a unified interoperable interface.
%
To implement \emph{heterogeneous} \acp{DTE} the thesis explores architectural patterns and best practices taking inspiration from the (semantic) Web, as an example of large-scale decentralized system that successfully integrates heterogeneous resources and supports their interaction through standardized protocols and data formats~\cite{Shadbolt_Berners-Lee_Hall_2006}.
%
Namely, the approach is grounded on \emph{hypermedia} representations that can semantically describe the assets and their relationships, and support navigation and interaction through hyperlinks~\cite{fielding2000architectural}.
%
To support the development of intelligent applications, this work refers to \ac{MAS} as an engineering paradigm for autonomous systems which model distributed applications in dynamic environments through sets of autonomous entities~\cite{2009wooldridge}.
Specifically, the thesis focuses on cognitive approaches such as the \ac{BDI} model~\cite{Bratman1987-BRAIPA,rao1991modeling}, as they merge adaptability and responsiveness with a declarative encoding of domain knowledge and business rules.
...
Towards this goal, the thesis aims to answer these research questions:

\begin{enumerate}[label=\textbf{RQ\arabic*}]

  \item\label{rq:1}
  \emph{How to engineer \acp{DT} which integrate heterogeneous data sources and provide an up-to-date uniform representation of the mirrored assets?}
  \newline
  ...
  \item\label{rq:2}
  \emph{How to engineer \acp{DTE} which integrate heterogeneous \acp{DT} and support applications in interacting with them?}
  \newline
  Given the fragmented nature of \acp{DT} it is essential to ensure that \acp{DTE} can integrate heterogeneous \acp{DT} and offer features that facilitate the interaction of external applications with the set of \acp{DT} within an ecosystem.

  \item\label{rq:3}
  \emph{How to support the operational management of \acp{DTE}?}
  \newline
  ...
  \newline
  Given the dynamic nature of \acp{DTE}, applications interacting with \acp{DT} need to be flexible, and encapsulate strategies and rules to automate decision-making. This thesis explores how (cognitive) \acp{MAS} can be used to this purpose and in general how to exploit both autonomous agents and \acp{DT} to engineer intelligent cyber-physical applications.  


  \item\label{rq:5}
  \emph{How to make the development of \ac{BDI} agents more accessible and easier to integrate with external systems such as \ac{DT}?}
  \newline
  \ac{BDI} agents are a powerful tool for modeling intelligent applications, but their development is often complex as they require a deep understanding of the underlying concepts. This work explores how to make the development of \ac{BDI} agents more accessible and easier to integrate with external systems looking at the design of new tools and frameworks.
\end{enumerate}
...
The central hypothesis of this thesis can be summarized as follows:
...
To validate this hypothesis, the research questions are addressed with the following strategy:
\begin{itemize}
    \item \textbf{RQ1--2:} Investigate how heterogeneous \acp{DT} can be integrated and represented within a \ac{DTE} to provide a coherent, semantically-rich view.  
    \item \textbf{RQ3:} Focus on the operational management of the \ac{DTE}, ensuring its practical deployment and lifecycle support.  
    \item \textbf{RQ4--5:} Explore how multi-agent systems, and particularly \ac{BDI} agents, can leverage \acp{DTE} to develop intelligent, adaptive, and explainable applications.  
\end{itemize}

By investigating this hypothesis, the thesis aims to demonstrate that a structured approach to \ac{DTE} engineering can effectively combine interoperability, real-time synchronization, and intelligent autonomous behavior, forming a foundation for robust and adaptable cyber-physical applications.
%
The validation of the hypothesis is achieved through the definition of architectural frameworks, patterns, and prototype implementations, as well as the application of the proposed approach to real-world healthcare scenarios provided by the industrial partner \ausl{}.
...
Here are the key contributions of this work to the area of \ac{DT} engineering:
...
\begin{itemize}
  ...
  \item Definition of the concept of \acp{DTE} and its role in the context of \ac{DT}-based systems~\cite{DBLP:journals/fgcs/GiulianelliBCR26}.

  \item Proposal for a framework to implement \acp{DTE} that can integrate heterogeneous \acp{DT} and expose a unified interface for external applications using semantic hypermedia representations~\cite{DBLP:journals/fgcs/GiulianelliBCR26,DBLP:conf/models/GiulianelliBCR24,DBLP:conf/models/BurattiniZPR24}.
...
\Cref{part:background} provides the necessary context and background on the key concepts that underpin this research, including an overview of the challenges of healthcare digitalization and the reality of \ausl{} in \Cref{chap:back:health4.0},
...
\Cref{part:dte} delves into the contributions towards the definition of the concept of \ac{DTE}. Namely, \Cref{chap:dte:engineering-dt} presents architectural proposals and patterns for the development of \ac{DT} that can be integrated into a \ac{DTE}, then \Cref{chap:dte:dte} introduces the concept of \ac{DTE}, and explore its operational management and prototype implementation through the integration of heterogeneous \acp{DT} in \Cref{chap:dte:dtc} and \Cref{chap:dte:hwodt} respectively.

\Cref{part:mas} focuses on the role of multi-agent systems in the development of intelligent applications discussing their alignment for a synergetic exploitation with \acp{DTE} in \Cref{chap:mas:mas-dt} and the contributions to the engineering of \ac{BDI} agents towards the goal of making them more accessible for developers and users in \Cref{chap:mas:engineering}.

Finally, \Cref{part:applications} presents application scenarios proposed by the industrial partner \ausl{}---namely, pharmaceutical supply chain (\Cref{chap:app:irst}), trauma management (\Cref{chap:app:trauma}), and operating room management (\Cref{chap:app:orm})---to demonstrate the applicability of the proposed approach to \ac{DTE} engineering in real-world healthcare settings,
